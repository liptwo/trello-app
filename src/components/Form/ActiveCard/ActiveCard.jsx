...
const SidebarItem = styled(Box)(({ theme }) => ({
  ...
  '&.active': {
    ...
    '&:hover': {
      backgroundColor:
        theme.palette.mode === 'dark' ? '#33485D' : theme.palette.grey[100]
    }
  }
}))
...
const ActiveCard = () => {
  ...
  const onUpdateCardMember = async (addMemberData) => {
    // memberToAdd = { memberId: '...', action: 'add' | 'remove' }
    await callApiUpdateCard({ addMemberData })
    // await callApiUpdateCard({ memberToAdd })
  }
  ...
  const handleJoin = () => {
    ...
    onUpdateCardMember(addMemberData)
  }
  return (
    <Modal
      disableScrollLock
      ...
      >
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
          ...
          <Box sx={{ flex: 3 }}>
            <Box sx={{ mb: 3 }}>
              <Typography
                sx={{ fontheight: '600', color: 'primary.main', mb: 1 }}
              >
                Members
              </Typography>
          ...
          <Box sx={{ flex: 1 }}>
            ...
            <Stack spacing={1} sx={{ width: '100%' }}>
              ...
              {!activeCard?.memberIds?.includes(currentUser._id) && (
                <SidebarItem onClick={handleJoin} className='active'>
